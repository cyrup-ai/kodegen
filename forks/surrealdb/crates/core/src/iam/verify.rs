...
fn pem_to_der(pem: &[u8]) -> Result<Vec<u8>> {
	let pem_str = std::str::from_utf8(pem)
		.map_err(|e| anyhow::anyhow!("Invalid UTF-8 in PEM: {}", e))?;
	// Remove BEGIN and END lines, strip whitespace, and decode base64
	let pem_cleaned = pem_str
		.lines()
...
fn decode_key(alg: catalog::Algorithm, key: &[u8]) -> Result<(DecodingKey, Validation)> {
	let (dec, mut val) = match alg {
		...
		catalog::Algorithm::EdDSA => {
			...
			(DecodingKey::from_ed_der(&der), Validation::new(jsonwebtoken::Algorithm::EdDSA))
		}
		catalog::Algorithm::Es256 => {
			let der = pem_to_der(key)?;
			(DecodingKey::from_ec_der(&der), Validation::new(jsonwebtoken::Algorithm::ES256))
		}
		catalog::Algorithm::Es384 => {
			let der = pem_to_der(key)?;
			(DecodingKey::from_ec_der(&der), Validation::new(jsonwebtoken::Algorithm::ES384))
		}
		...
		catalog::Algorithm::Ps256 => {
			let der = pem_to_der(key)?;
			(DecodingKey::from_rsa_der(&der), Validation::new(jsonwebtoken::Algorithm::PS256))
		}
		catalog::Algorithm::Ps384 => {
			let der = pem_to_der(key)?;
			(DecodingKey::from_rsa_der(&der), Validation::new(jsonwebtoken::Algorithm::PS384))
		}
		catalog::Algorithm::Ps512 => {
			let der = pem_to_der(key)?;
			(DecodingKey::from_rsa_der(&der), Validation::new(jsonwebtoken::Algorithm::PS512))
		}
		catalog::Algorithm::Rs256 => {
			let der = pem_to_der(key)?;
			(DecodingKey::from_rsa_der(&der), Validation::new(jsonwebtoken::Algorithm::RS256))
		}
		catalog::Algorithm::Rs384 => {
			let der = pem_to_der(key)?;
			(DecodingKey::from_rsa_der(&der), Validation::new(jsonwebtoken::Algorithm::RS384))
		}
		catalog::Algorithm::Rs512 => {
			let der = pem_to_der(key)?;
			(DecodingKey::from_rsa_der(&der), Validation::new(jsonwebtoken::Algorithm::RS512))
		}
	};
