// Category SSE Server: Introspection Tools
//
// This binary serves only introspection tools over SSE/HTTPS transport.
// Managed by kodegend daemon, typically running on port 30443.
...
#[derive(Parser, Debug)]
#[command(name = "kodegen-introspection")]
#[command(about = "Introspection tools SSE server")]
struct Args {
    /// SSE server bind address (e.g., "127.0.0.1:30443")
    #[arg(long)]
    sse: SocketAddr,
...
/// Introspection SSE Server handler
#[derive(Clone)]
...
impl ServerHandler for IntrospectionServer {
    fn get_info(&self) -> ServerInfo {
        ServerInfo {
            ...
            instructions: Some("KODEGEN Introspection Category Server".to_string()),
        }
    }
...
async fn main() -> Result<()> {
    ...
    let instance_id = chrono::Utc::now().format("%Y%m%d-%H%M%S-introspection").to_string();
    ...
    // Build routers with only introspection tools
    let tool_router = ToolRouter::new();
    let prompt_router = PromptRouter::new();

    // Register all introspection tools
    let (tool_router, prompt_router) = register_introspection_tools(
        tool_router,
        prompt_router,
        &config_manager,
        &usage_tracker,
    ...
    let server = IntrospectionServer {
        tool_router,
        prompt_router,
        usage_tracker,
        config_manager,
    ...
    log::info!("Starting introspection category SSE server on {protocol}://{}", args.sse);
    ...
    log::info!("Introspection server stopped");
    Ok(())
}

/// Register all introspection tools into routers
fn register_introspection_tools<S>(
...
where
    S: Send + Sync + 'static,
{
    use kodegen_tools_introspection::*;
    use std::sync::Arc;
    ...
    (tool_router, prompt_router) = register(
        tool_router,
        prompt_router,
        GetUsageStatsTool::new(usage_tracker.clone()),
    );
    (tool_router, prompt_router) = register(
        tool_router,
        prompt_router,
        GetRecentToolCallsTool::new(),
    );
