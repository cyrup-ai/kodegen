...
pub fn import_p12_with_lock(
    ...
    let lock_dir = dirs::cache_dir()
        .ok_or_else(|| SetupError::MissingConfig("Could not determine cache directory".to_string()))?
        .join("kodegen");
    ...
    lock_file.lock_exclusive()
        .map_err(|e| SetupError::KeychainOperation(format!("Failed to acquire keychain lock: {e}")))?;
    ...
    let check_output = Command::new("security")
        ...
        .output()
        .map_err(|e| SetupError::CommandExecution(format!("Failed to check existing certificate: {e}")))?;
    ...
    let import_output = Command::new("security")
        .args([
            "import",
            p12_path.to_str().ok_or_else(|| SetupError::InvalidConfig("Invalid p12 path".to_string()))?,
            "-k", keychain,
            "-P", password,
            "-T", "/usr/bin/codesign"
        ])
        .output()
        .map_err(|e| SetupError::KeychainOperation(format!("Failed to execute security import: {e}")))?;
    
    if !import_output.status.success() {
        let stderr = String::from_utf8_lossy(&import_output.stderr);
        return Err(SetupError::KeychainOperation(format!("Keychain import failed: {stderr}")));
    }
...
pub fn check_for_developer_certificates(output: std::process::Output) -> Result<Vec<String>> {
    ...
    let output_str = std::str::from_utf8(&output.stdout)
        .map_err(|e| SetupError::CommandExecution(format!(
            "security command output is not valid UTF-8: {e}"
        )))?;
...
pub fn check_certificate_expiry(cert_name: &str) -> Result<CertificateInfo> {
    ...
    let expiry_str = date_str.strip_prefix("notAfter=")
...
fn get_cert_hash(cert_path: &Path) -> Result<String> {
    ...
    let hash = hash_line.strip_prefix("SHA1 Fingerprint=")
...
impl TempKeychain {
    ...
    pub fn from_certificate_bytes(
        ...
        let keychain_name = format!("{}.keychain-db",
        ...
        // Create keychain
        run_security(&["create-keychain", "-p", &keychain_password],
                     &keychain_path)?;
        
        // Unlock keychain
        run_security(&["unlock-keychain", "-p", &keychain_password],
                     &keychain_path)?;
        
        // Import Apple's Developer ID G2 CA certificate first (required for trust chain)
        let g2_ca_result = Command::new("curl")
            .args(["-s", "https://www.apple.com/certificateauthority/DeveloperIDG2CA.cer"])
            .output();
        ...
        run_security(&["set-keychain-settings", "-t", "3600", "-u"],
                     &keychain_path)?;
...
impl Drop for TempKeychain {
    fn drop(&mut self) {
        ...
        match result {
            ...
            Ok(output) => {
                eprintln!("⚠️  Failed to delete temporary keychain: {}",
                    String::from_utf8_lossy(&output.stderr));
            }
            Err(e) => {
...
fn run_security(args: &[&str], keychain_path: &Path) -> Result<()> {
    ...
    cmd_args.push(keychain_path.to_str()
...
pub fn sign_with_entitlements(
    ...
    if !binary_path.exists() {
        return Err(SetupError::InvalidConfig(format!(
            "Binary not found: {}", binary_path.display()
        )));
    }
    ...
    if let Some(entitlements) = entitlements_path {
        if !entitlements.exists() {
            return Err(SetupError::InvalidConfig(format!(
                "Entitlements file not found: {}", entitlements.display()
            )));
        }
        
        args.push("--entitlements");
        args.push(entitlements.to_str()
            .ok_or_else(|| SetupError::InvalidConfig(
                "Invalid entitlements path".to_string()
            ))?);
    }
