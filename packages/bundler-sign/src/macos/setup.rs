...
pub fn interactive_setup() -> Result<()> {
    ...
    let key_path = if let Some(path) = prompt_for_p8_path()? {
        path
    } else {
    ...
    let client =
        apple_api::AppleAPIClient::new(key_id, issuer_id, std::path::Path::new(&key_path))?;
...
fn continue_setup_from_config(
    ...
    if dry_run {
        ...
        let client = apple_api::AppleAPIClient::new(&config.key_id, &config.issuer_id, key_path)?;
