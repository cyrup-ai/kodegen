...
impl AppleAPIClient {
    ...
    pub fn request_certificate(&self, csr_pem: &str, cert_type: CertificateType) -> Result<Vec<u8>> {
        ...
        for attempt in 1..=MAX_RETRY_ATTEMPTS {
            ...
            match self.request_certificate_inner(csr_pem, cert_type) {
        ...
        Err(last_error.unwrap_or_else(|| anyhow::anyhow!("Certificate request failed after {MAX_RETRY_ATTEMPTS} attempts")))
    }

    /// Inner implementation of certificate request
    fn request_certificate_inner(&self, csr_pem: &str, cert_type: CertificateType) -> Result<Vec<u8>> {
        ...
        if !response.status().is_success() {
            ...
            let detailed_error = if let Ok(err_resp) = serde_json::from_str::<AppleErrorResponse>(&error_text) {
...
fn is_retryable_error(error: &anyhow::Error) -> bool {
    ...
    msg.contains("timeout") || msg.contains("connection")
        || msg.contains("50") || msg.contains("429")
}
...
fn validate_certificate_der(cert_der: &[u8]) -> Result<()> {
    ...
    if cert_der[0] != 0x30 {
        anyhow::bail!("Invalid DER format (expected 0x30, got 0x{:02x})", cert_der[0]);
    }
