...
pub struct RegistrationConfig<'a> {
    pub config_manager: &'a kodegen_tools_config::ConfigManager,
    pub usage_tracker: &'a UsageTracker,
    pub enabled_categories: &'a Option<HashSet<String>>,
    pub database_dsn: Option<&'a str>,
    #[cfg(feature = "database")]
    pub ssh_config: Option<(
...
pub async fn register_all_tools<S>(
    ...
    config: RegistrationConfig<'_>,
) -> Result<(
    ToolRouter<S>,
    PromptRouter<S>,
    crate::common::router_builder::Managers,
)>
where
    S: Send + Sync + 'static,
{
    let config_manager = config.config_manager;
    let _usage_tracker = config.usage_tracker;
    let enabled_categories = config.enabled_categories;
    let _database_dsn = config.database_dsn;
    let _ssh_config = config.ssh_config;
    ...
    if let Some(dsn) = _database_dsn {
        ...
        use kodegen_tools_database::{
            ExposeSecret, SecretString, establish_tunnel, rewrite_dsn_for_tunnel,
        };
    ...
    if is_category_enabled("filesystem", enabled_categories) {
        (tool_router, prompt_router) =
            register_filesystem_tools(tool_router, prompt_router, config_manager).await?;
    }
    ...
    if is_category_enabled("terminal", enabled_categories) {
        (tool_router, prompt_router) =
            register_terminal_tools(tool_router, prompt_router, config_manager).await?;
    }
    ...
    if is_category_enabled("introspection", enabled_categories) {
        (tool_router, prompt_router) =
            register_introspection_tools(tool_router, prompt_router, _usage_tracker).await?;
    }
    ...
    if is_category_enabled("sequential_thinking", enabled_categories) {
        (tool_router, prompt_router) =
            register_sequential_thinking_tool(tool_router, prompt_router).await?;
    }
    ...
    if is_category_enabled("claude_agent", enabled_categories) {
        (tool_router, prompt_router) =
            register_claude_agent_tools(tool_router, prompt_router).await?;
    }
    ...
    if is_category_enabled("citescrape", enabled_categories) {
        let browser_manager;
        (tool_router, prompt_router, browser_manager) =
            register_citescrape_tools(tool_router, prompt_router).await?;
    ...
    if is_category_enabled("config", enabled_categories) {
        (tool_router, prompt_router) =
            register_config_tools(tool_router, prompt_router, config_manager).await?;
    }
    ...
    if is_category_enabled("browser", enabled_categories) {
        ...
        (tool_router, prompt_router, browser_tools_manager) =
            register_browser_tools(tool_router, prompt_router, _server_url).await?;
    ...
    S: Send + Sync + 'static,
{
    ...
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_filesystem::WriteFileTool::new(config_manager.clone()),
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_filesystem::MoveFileTool::new(config_manager.clone()),
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_filesystem::DeleteFileTool::new(config_manager.clone()),
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_filesystem::DeleteDirectoryTool::new(config_manager.clone()),
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_filesystem::ListDirectoryTool::new(config_manager.clone()),
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_filesystem::CreateDirectoryTool::new(config_manager.clone()),
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_filesystem::GetFileInfoTool::new(config_manager.clone()),
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_filesystem::EditBlockTool::new(config_manager.clone()),
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
    ...
    S: Send + Sync + 'static,
{
    ...
    let command_manager =
        kodegen_tools_terminal::CommandManager::new(config_manager.get_blocked_commands());
    ...
    S: Send + Sync + 'static,
{
    ...
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_introspection::GetUsageStatsTool::new(usage_tracker.clone()),
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
    ...
    S: Send + Sync + 'static,
{
    ...
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_prompt::AddPromptTool::new().await?,
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_prompt::EditPromptTool::new().await?,
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_prompt::DeletePromptTool::new().await?,
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_prompt::GetPromptTool::new().await?,
    );
    ...
    S: Send + Sync + 'static,
{
    ...
    let (tool_router, prompt_router) =
        register_tool_arc(tool_router, prompt_router, thinking_tool.clone());
    ...
    S: Send + Sync + 'static,
{
    ...
    prompt_manager
        .init()
        .await
        .map_err(|e| anyhow::anyhow!("Failed to init prompt manager: {e}"))?;

    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_claude_agent::SpawnClaudeAgentTool::new(
            agent_manager.clone(),
    ...
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_claude_agent::SendClaudeAgentPromptTool::new(
            agent_manager.clone(),
            prompt_manager.clone(),
        ),
    );
...
async fn register_citescrape_tools<S>(
    ...
    ToolRouter<S>,
    PromptRouter<S>,
    Arc<kodegen_tools_citescrape::BrowserManager>,
)>
where
    S: Send + Sync + 'static,
{
    ...
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_citescrape::StartCrawlTool::new(
    ...
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_citescrape::SearchCrawlResultsTool::new(
            session_manager.clone(),
            engine_cache.clone(),
        ),
    );
    ...
    S: Send + Sync + 'static,
{
    ...
    let (tool_router, prompt_router) =
        register_tool(tool_router, prompt_router, kodegen_tools_git::GitInitTool);
    let (tool_router, prompt_router) =
        register_tool(tool_router, prompt_router, kodegen_tools_git::GitOpenTool);
    let (tool_router, prompt_router) =
        register_tool(tool_router, prompt_router, kodegen_tools_git::GitCloneTool);
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_git::GitDiscoverTool,
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_git::GitBranchCreateTool,
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_git::GitBranchDeleteTool,
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_git::GitBranchListTool,
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_git::GitBranchRenameTool,
    );
    let (tool_router, prompt_router) =
        register_tool(tool_router, prompt_router, kodegen_tools_git::GitCommitTool);
    let (tool_router, prompt_router) =
        register_tool(tool_router, prompt_router, kodegen_tools_git::GitLogTool);
    let (tool_router, prompt_router) =
        register_tool(tool_router, prompt_router, kodegen_tools_git::GitAddTool);
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_git::GitCheckoutTool,
    );
    let (tool_router, prompt_router) =
        register_tool(tool_router, prompt_router, kodegen_tools_git::GitFetchTool);
    let (tool_router, prompt_router) =
        register_tool(tool_router, prompt_router, kodegen_tools_git::GitMergeTool);
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_git::GitWorktreeAddTool,
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_git::GitWorktreeRemoveTool,
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_git::GitWorktreeListTool,
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_git::GitWorktreeLockTool,
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_git::GitWorktreeUnlockTool,
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_git::GitWorktreePruneTool,
    );
    ...
    S: Send + Sync + 'static,
{
    ...
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_github::CreateIssueTool,
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_github::GetIssueTool,
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_github::ListIssuesTool,
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_github::UpdateIssueTool,
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_github::SearchIssuesTool,
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_github::AddIssueCommentTool,
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_github::GetIssueCommentsTool,
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_github::CreatePullRequestTool,
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_github::UpdatePullRequestTool,
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_github::MergePullRequestTool,
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_github::GetPullRequestStatusTool,
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_github::GetPullRequestFilesTool,
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_github::GetPullRequestReviewsTool,
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_github::CreatePullRequestReviewTool,
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_github::AddPullRequestReviewCommentTool,
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_github::RequestCopilotReviewTool,
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_github::CreateRepositoryTool,
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_github::ForkRepositoryTool,
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_github::ListBranchesTool,
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_github::CreateBranchTool,
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_github::ListCommitsTool,
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_github::GetCommitTool,
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_github::SearchCodeTool,
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_github::SearchRepositoriesTool,
    );
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_github::SearchUsersTool,
    );
    ...
    S: Send + Sync + 'static,
{
    ...
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_database::tools::ExecuteSQLTool::new(
            pool.clone(),
            config_manager.clone(),
            connection_url,
        )?,
    ...
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_database::tools::ListSchemasTool::new(
            pool.clone(),
            connection_url,
            config_manager.clone(),
        )?,
    ...
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_database::tools::ListTablesTool::new(
            pool.clone(),
            connection_url,
            config_manager.clone(),
        )?,
    ...
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_database::tools::GetTableSchemaTool::new(
            pool.clone(),
            connection_url,
    ...
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_database::tools::GetTableIndexesTool::new(
            pool.clone(),
            connection_url,
            Arc::new(config_manager.clone()),
        )?,
    ...
    let (tool_router, prompt_router) = register_tool(
        tool_router,
        prompt_router,
        kodegen_tools_database::tools::GetStoredProceduresTool::new(
            pool.clone(),
            connection_url,
            Arc::new(config_manager.clone()),
        )?,
...
async fn register_browser_tools<S>(
    ...
    ToolRouter<S>,
    PromptRouter<S>,
    Arc<kodegen_tools_browser::BrowserManager>,
)>
where
    S: Send + Sync + 'static,
{
    ...
    // Register BrowserResearchTool if server_url is provided
    #[cfg(feature = "research")]
    let (tool_router, prompt_router) = if let Some(url) = server_url {
        log::debug!("Registering BrowserResearchTool with loopback URL: {}", url);
        register_tool(
            tool_router,
            prompt_router,
            kodegen_tools_browser::BrowserResearchTool::new(url.to_string()),
        )
    } else {
        log::debug!("Skipping BrowserResearchTool registration (no server URL provided)");
        (tool_router, prompt_router)
    };

    // Register all other browser tools
