...
pub async fn build_routers<S>(
    ...
    #[cfg(feature = "database")] ssh_config: Option<(
        kodegen_tools_database::SSHConfig,
        kodegen_tools_database::TunnelConfig,
    )>,
    #[cfg(not(feature = "database"))] ssh_config: Option<()>,
    ...
    S: Send + Sync + 'static,
{
    ...
    let config = crate::common::tool_registry::RegistrationConfig {
        config_manager,
        usage_tracker,
        enabled_categories,
        database_dsn,
    ...
    let (tool_router, prompt_router, managers) =
        crate::common::tool_registry::register_all_tools(tool_router, prompt_router, config)
            .await?;
