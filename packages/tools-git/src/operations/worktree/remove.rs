...
fn worktree_remove_impl(repo: gix::Repository, opts: WorktreeRemoveOpts) -> GitResult<()> {
    ...
    let is_main = proxy.git_dir()
        .parent()
        .and_then(|p| p.file_name()).is_none_or(|n| n != "worktrees");
    ...
    if worktree_path.exists() {
        std::fs::remove_dir_all(&worktree_path).map_err(|e| {
            GitError::Io(std::io::Error::new(
                e.kind(),
                format!("Failed to remove worktree directory at {}: {}", 
    ...
    std::fs::remove_dir_all(&git_dir).map_err(|e| {
        GitError::Io(std::io::Error::new(
            e.kind(),
            format!("Failed to remove worktree git directory at {}: {}", 
                    git_dir.display(), e)
        ))
    })?;
