...
pub async fn delete_remote_tag(repo: &RepoHandle, remote: &str, tag_name: &str) -> GitResult<()> {
    ...
    if tag_name.is_empty() {
        return Err(GitError::InvalidInput("Tag name cannot be empty".to_string()));
    }
    if tag_name.contains("..") {
        return Err(GitError::InvalidInput(format!("Invalid tag name: {tag_name}")));
    }
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        return Err(GitError::InvalidInput(format!("Failed to delete remote tag '{tag_name_owned}': {stderr}")));
    }
...
/// # }
/// ```
pub async fn delete_remote_branch(repo: &RepoHandle, remote: &str, branch_name: &str) -> GitResult<()> {
    let work_dir = repo.raw().workdir()
        .ok_or_else(|| GitError::InvalidInput("Repository has no working directory".to_string()))?
        .to_path_buf();
    ...
    if branch_name.is_empty() {
        return Err(GitError::InvalidInput("Branch name cannot be empty".to_string()));
    }
    if branch_name.contains("..") {
        return Err(GitError::InvalidInput(format!("Invalid branch name: {branch_name}")));
    }
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        return Err(GitError::InvalidInput(format!("Failed to delete remote branch '{branch_name_owned}': {stderr}")));
    }
...
/// # }
/// ```
pub async fn check_remote_branch_exists(repo: &RepoHandle, remote: &str, branch_name: &str) -> GitResult<bool> {
    let work_dir = repo.raw().workdir()
        .ok_or_else(|| GitError::InvalidInput("Repository has no working directory".to_string()))?
        .to_path_buf();
...
/// # }
/// ```
pub async fn check_remote_tag_exists(repo: &RepoHandle, remote: &str, tag_name: &str) -> GitResult<bool> {
    let work_dir = repo.raw().workdir()
        .ok_or_else(|| GitError::InvalidInput("Repository has no working directory".to_string()))?
        .to_path_buf();
