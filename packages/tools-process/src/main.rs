// Category SSE Server: Process Tools
//
// This binary serves only process management tools over SSE/HTTPS transport.
// Managed by kodegend daemon, typically running on port 30442.
...
#[derive(Parser, Debug)]
#[command(name = "kodegen-process")]
#[command(about = "Process management tools SSE server")]
struct Args {
    /// SSE server bind address (e.g., "127.0.0.1:30442")
    #[arg(long)]
    sse: SocketAddr,
...
impl ServerHandler for ProcessServer {
    fn get_info(&self) -> ServerInfo {
        ServerInfo {
            ...
            instructions: Some("KODEGEN Process Category Server".to_string()),
        }
    }
...
async fn main() -> Result<()> {
    ...
    let instance_id = chrono::Utc::now().format("%Y%m%d-%H%M%S-process").to_string();
    ...
    // Build routers with only process tools
    let tool_router = ToolRouter::new();
    let prompt_router = PromptRouter::new();

    // Register all process tools
    let (tool_router, prompt_router) = register_process_tools(
        tool_router,
        prompt_router,
        &config_manager,
        &usage_tracker,
    ...
    log::info!("Starting process category SSE server on {protocol}://{}", args.sse);
    ...
    log::info!("Process server stopped");
    Ok(())
}

/// Register all process tools into routers
fn register_process_tools<S>(
