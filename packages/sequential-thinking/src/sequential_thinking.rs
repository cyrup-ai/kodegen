...
impl SequentialThinkingTool {
    ...
    async fn try_restore_session(&self, session_id: &str) -> Option<SessionHandle> {
        ...
        log::debug!("Attempting to restore session {} from disk", session_id);
        ...
        for idx in 1..=metadata.total_thoughts {
            let thought_path = session_dir.join(format!("thought{}.json", idx));
        ...
        for branch_id in &metadata.branch_ids {
            ...
            let mut idx = 1;
            loop {
                let branch_path = session_dir.join(format!("branch_{}_thought{}.json", branch_id, idx));
    ...
    async fn cleanup_sessions(&self, max_age: Duration) {
        ...
        sessions.retain(|session_id, handle| {
            ...
            // Check last activity
            let last_activity = handle.last_activity.try_read()
                .map(|t| *t)
        ...
        Ok(vec![
            PromptMessage {
                ...
                content: PromptMessageContent::text(
                    "How do I use the sequentialthinking tool to solve a complex problem?"
                ),
            },
            PromptMessage {
                role: PromptMessageRole::Assistant,
                content: PromptMessageContent::text(
                    "The sequentialthinking tool helps you break down complex problems step by step:\n\n\
