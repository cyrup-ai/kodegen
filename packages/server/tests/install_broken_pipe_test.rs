...
fn test_install_handles_broken_pipe() {
    ...
    assert!(
        ...
        "stderr should contain warning about write failure. Got: {}",
        stderr
    );
    ...
    assert!(
        ...
        "stderr should contain fallback install results. Got: {}",
        stderr
    );
    ...
    assert!(
        has_result_markers,
        "stderr should contain result markers (✓ or ✗) in fallback output. Got: {}",
        stderr
    );
}
...
fn test_install_with_immediate_stdout_close() {
    ...
    assert!(
        ...
        "stderr should contain fallback output or warning. Got: {}",
        stderr
    );
}
