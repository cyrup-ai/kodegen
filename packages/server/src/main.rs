...
/// Check if a tool category should be enabled at runtime
/// 
/// Returns true if:
/// - No filter specified (enabled_categories is None), OR
/// - Filter specified and category is in the filter
fn is_category_enabled(category: &str, enabled_categories: &Option<HashSet<String>>) -> bool {
    match enabled_categories {
        None => true,  // No filter - enable all compiled categories
        Some(set) => set.contains(category),  // Filter specified - check membership
    }
}

/// Initialize and return tool and prompt routers (async for prompt tool initialization)
pub async fn build_routers(
    config_manager: &kodegen_config::ConfigManager,
    usage_tracker: &UsageTracker,
    enabled_categories: &Option<HashSet<String>>,
) -> Result<(ToolRouter<StdioServer>, PromptRouter<StdioServer>)> {
    // Log what's enabled
    match enabled_categories {
        None => {
        ...
        Some(set) => {
            ...
            for requested in set {
                if !cli::available_categories().contains(&requested.as_str()) {
                    log::warn!(
    ...
    // Instantiate filesystem tools (14 tools) - CONDITIONAL
    #[cfg(feature = "filesystem")]
    let filesystem_tools = if is_category_enabled("filesystem", enabled_categories) {
        log::debug!("Initializing filesystem tools");
        Some({
            // Create search manager
            let search_manager = std::sync::Arc::new(kodegen_filesystem::search::SearchManager::new(config_manager.clone()));
            search_manager.clone().start_cleanup_task();
            
            (
                Arc::new(kodegen_filesystem::ReadFileTool::new(config_manager.get_file_read_line_limit(), config_manager.clone())),
                Arc::new(kodegen_filesystem::ReadMultipleFilesTool::new(config_manager.get_file_read_line_limit(), config_manager.clone())),
                Arc::new(kodegen_filesystem::WriteFileTool::new(config_manager.clone())),
                Arc::new(kodegen_filesystem::MoveFileTool::new(config_manager.clone())),
...
async fn async_main() -> Result<()> {
    ...
    // Build routers (async)
    let (tool_router, prompt_router) = build_routers(
        &config_manager,
        &usage_tracker,
        &enabled_categories,
    ).await?;
