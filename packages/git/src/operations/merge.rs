...
pub fn merge(repo: RepoHandle, opts: MergeOpts) -> AsyncTask<GitResult<MergeOutcome>> {
    ...
    AsyncTask::spawn(move || {
        ...
        let parsed = repo.rev_parse(theirs.as_bytes().as_bstr()).map_err(|e| {
        ...
        let mut head_ref = repo.head().map_err(|e| GitError::Gix(e.into()))?;
        ...
        if could_fast_forward && !no_ff {
            fast_forward_merge(&repo, their_commit_id_detached)?;
