...
pub fn fetch(repo: RepoHandle, opts: FetchOpts) -> AsyncTask<GitResult<()>> {
    ...
    AsyncTask::spawn(move || {
        ...
        let remote_ref = repo
            ...
            .map_err(|e| GitError::InvalidInput(format!("Remote '{}' not found: {}", remote, e)))?;
        ...
        if prune {
            prune_stale_refs(&repo, &remote_name, &outcome.ref_map)?;
        }
