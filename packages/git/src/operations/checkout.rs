...
fn checkout_files(
    ...
    let object_id = parsed.single().ok_or_else(|| {
        GitError::InvalidInput(format!(
            "Reference '{}' is ambiguous (matches multiple objects)",
            reference
        ))
    })?;
    ...
    // Step 7: Write index to disk
    let index_path = repo.index_path();
    let mut file = std::fs::File::create(&index_path).map_err(|e| {
        GitError::Io(std::io::Error::new(
            e.kind(),
            format!(
    ...
    index.write_to(&mut file, Options::default()).map_err(|e| {
        GitError::Gix(
            format!("Failed to write index to {}: {}", index_path.display(), e).into(),
        )
    })?;
...
pub fn checkout(repo: RepoHandle, opts: CheckoutOpts) -> AsyncTask<GitResult<()>> {
    ...
    AsyncTask::spawn(move || {
        ...
        if let Some(file_paths) = paths {
            return checkout_files(&repo, &reference, file_paths, force);
        }
        ...
        let commit = repo
            ...
            .map_err(|_| {
                GitError::InvalidInput(format!(
                    "Reference '{}' (oid: {}) does not point to a commit",
                    reference, object_id
                ))
            })?;
        ...
        if !outcome.errors.is_empty() {
            let first_error = &outcome.errors[0];
            return Err(GitError::Gix(
                format!(
                    "Checkout failed with {} error(s). First error: {}. Working directory may be in a partial state.",
                    outcome.errors.len(),
                    first_error.error
                )
                .into(),
            ...
            let collision_paths: Vec<String> = outcome
                ...
                .map(|entry| {
                    std::str::from_utf8(entry.path.as_ref())
                        .map(|s| s.to_string())
                        .unwrap_or_else(|_| format!("{:?}", entry.path))
                })
                .collect();
        ...
        // Step 9: Write updated index to disk
        let index_path = repo.index_path();
        let mut file = std::fs::File::create(&index_path).map_err(|e| {
            GitError::Io(std::io::Error::new(
                e.kind(),
        ...
        let is_local_branch = if reference.starts_with("refs/heads/") {
            ...
            repo.try_find_reference(reference.as_bytes().as_bstr())
            ...
            repo.try_find_reference(full_ref_name.as_bytes().as_bstr())
                .map_err(|e| GitError::Gix(format!("Failed to check reference: {}", e).into()))?
                .is_some()
        } else {
