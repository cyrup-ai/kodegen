...
fn worktree_add_impl(repo: gix::Repository, opts: WorktreeAddOpts) -> GitResult<PathBuf> {
    ...
    let commit_id = parsed.single().ok_or_else(|| {
        GitError::InvalidInput(format!(
            "Committish '{}' does not resolve to a single commit",
            committish_ref
        ))
    })?;
    ...
    let mut file = match std::fs::File::create(&index_path) {
        Ok(f) => f,
        Err(e) => {
            cleanup();
            return Err(GitError::Io(std::io::Error::new(
                e.kind(),
                format!("Failed to create index file at {}: {}", index_path.display(), e),
            )));
        }
    };

    if let Err(e) = index.write_to(&mut file, gix::index::write::Options::default()) {
        cleanup();
        return Err(GitError::Gix(
