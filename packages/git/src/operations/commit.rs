...
pub fn commit(repo: RepoHandle, opts: CommitOpts) -> AsyncTask<GitResult<CommitId>> {
    ...
    AsyncTask::spawn(move || {
        ...
        let index = if all {
            ...
            if changed {
                ...
                index
                    .write_to(&mut file, Options::default())
                    .map_err(|e| GitError::Gix(e.into()))?;
            }

            // Re-open index for tree building
            repo.open_index().map_err(|e| GitError::Gix(e.into()))?
        } else {
            index
        };
