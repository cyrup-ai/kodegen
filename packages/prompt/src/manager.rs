...
impl PromptManager {
    ...
    pub async fn load_prompt(&self, name: &str) -> Result<PromptTemplate> {
        ...
        let path = self.prompts_dir.join(format!("{}.j2.md", name));
    ...
    pub async fn add_prompt(&self, name: &str, content: &str) -> Result<()> {
        ...
        if fs::try_exists(&path).await.unwrap_or(false) {
            anyhow::bail!("Prompt '{}' already exists. Use edit_prompt to modify.", name);
        }
    ...
    pub async fn edit_prompt(&self, name: &str, content: &str) -> Result<()> {
        ...
        // Check exists (async)
        if !fs::try_exists(&path).await.unwrap_or(false) {
            anyhow::bail!("Prompt '{}' not found. Use add_prompt to create.", name);
        }
...
fn validate_prompt_name(name: &str) -> Result<()> {
    ...
    if !name.chars().all(|c| c.is_alphanumeric() || c == '-' || c == '_') {
        anyhow::bail!(
            "Invalid prompt name: '{}'. Only alphanumeric characters, hyphens, and underscores allowed.",
            name
        );
    }
    ...
    if name.contains('/') || name.contains('\\') || name.contains("..") {
        anyhow::bail!(
            "Invalid prompt name: '{}'. Path separators and '..' not allowed.",
            name
        );
    }
