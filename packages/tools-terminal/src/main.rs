// Category SSE Server: Terminal Tools
//
// This binary serves only terminal-related tools over SSE/HTTPS transport.
// Managed by kodegend daemon, typically running on port 30441.
...
#[derive(Parser, Debug)]
#[command(name = "kodegen-terminal")]
#[command(about = "Terminal tools SSE server")]
struct Args {
    /// SSE server bind address (e.g., "127.0.0.1:30441")
    #[arg(long)]
    sse: SocketAddr,
...
impl ServerHandler for TerminalServer {
    fn get_info(&self) -> ServerInfo {
        ServerInfo {
            ...
            instructions: Some("KODEGEN Terminal Category Server".to_string()),
        }
    }
...
async fn main() -> Result<()> {
    ...
    let instance_id = chrono::Utc::now().format("%Y%m%d-%H%M%S-terminal").to_string();
    ...
    // Build routers with only terminal tools
    let tool_router = ToolRouter::new();
    let prompt_router = PromptRouter::new();

    // Register all terminal tools
    let (tool_router, prompt_router) = register_terminal_tools(
        tool_router,
        prompt_router,
        &config_manager,
        &usage_tracker,
    ...
    log::info!("Starting terminal category SSE server on {protocol}://{}", args.sse);
...
/// Register all terminal tools into routers
fn register_terminal_tools<S>(
...
where
    S: Send + Sync + 'static,
{
    use kodegen_tools_terminal::*;
    use std::sync::Arc;

    let terminal_manager = Arc::new(TerminalManager::new());
    let command_manager = CommandManager::new(config_manager.get_blocked_commands());
    ...
    (tool_router, prompt_router) = register(
        tool_router,
        prompt_router,
        StartTerminalCommandTool::new(terminal_manager.clone(), command_manager),
    );
    (tool_router, prompt_router) = register(
        tool_router,
        prompt_router,
        ReadTerminalOutputTool::new(terminal_manager.clone()),
    );
    (tool_router, prompt_router) = register(
        tool_router,
        prompt_router,
        SendTerminalInputTool::new(terminal_manager.clone()),
    );
    (tool_router, prompt_router) = register(
        tool_router,
        prompt_router,
        StopTerminalCommandTool::new(terminal_manager.clone()),
    );
    (tool_router, prompt_router) = register(
        tool_router,
        prompt_router,
        ListTerminalCommandsTool::new(terminal_manager.clone()),
    );

    // CRITICAL: Start cleanup task after registration
