...
impl SearchManager {
    ...
    #[must_use]
    pub fn new(config_manager: kodegen_config::ConfigManager) -> Self {
        Self {
    ...
    pub async fn start_search(
        ...
        let effective_max_results = match options.max_results {
            ...
            Some(requested) => {
                ...
                if capped < requested {
                    log::warn!(
                        "Search max_results capped from {} to {} (absolute limit)",
                        requested,
                        capped
                    );
                }
        ...
        log::debug!(
            "Starting search with effective max_results: {}",
        ...
        let session_id = loop {
            ...
            collision_count += 1;
            log::error!(
                "UUID v4 collision #{} detected: {}. This indicates a serious problem with the RNG!",
                collision_count, id
            );
            ...
            tokio::spawn(async move {
                ...
                match timeout_result {
                    ...
                    Err(_elapsed) => {
                        ...
                        log::warn!("Search session {} timed out", session_id);
                        ...
                        if let Some(session) = sessions_guard.get(&session_id) {
                            ...
                            log::debug!("Timeout fired but session {} already cleaned up", session_id);
                        }
                    }
                }
            });
    ...
    pub async fn terminate_search(&self, session_id: &str) -> Result<bool, McpError> {
        ...
        if let Ok(()) = session.cancellation_tx.send(true) {
            ...
            log::debug!("Search session {} already finished", session_id);
            Ok(false)
        }
    }
    ...
    pub async fn cleanup_sessions(&self) {
        ...
        if cleaned_count > 0 {
            log::info!("Cleaned up {} search sessions", cleaned_count);
        }
    }
