...
pub(super) fn execute(
    ...
    use super::super::rg::flags::lowargs::{
        ...
        BoundaryMode, ContextMode, BinaryMode as RgBinaryMode, EncodingMode
    };
    use super::super::rg::flags::hiargs::HiArgs;
    use super::super::types::BinaryMode;
    ...
    let encoding_mode = match options.encoding.as_deref() {
        ...
        Some(enc_str) => {
            match grep::searcher::Encoding::new(enc_str) {
                ...
                Err(e) => {
                    log::warn!("Invalid encoding '{}': {}, using auto", enc_str, e);
                    EncodingMode::Auto
                }
            }
        }
    ...
    let mut low_args = LowArgs {
        ...
        boundary: if options.word_boundary {
        ...
        max_count: if ctx.output_mode == SearchOutputMode::FilesOnly {
            ...
            options.max_results.map(|n| n as u64)
        },
    ...
    super::utils::configure_walker(&mut walker, options);
