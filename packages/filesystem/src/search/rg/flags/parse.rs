...
fn parse_low() -> ParseResult<LowArgs> {
    // Initialize env_logger (ignores error if already initialized)
    ...
    // to re-parse.
    let config_args = crate::search::rg::flags::config::args();
    ...
    let mut low = LowArgs::default();
    parser.parse(final_args.into_iter(), &mut low)?;
...
fn set_log_levels(low: &LowArgs) {
    crate::search::rg::messages::set_messages(!low.no_messages);
    crate::search::rg::messages::set_ignore_messages(!low.no_ignore_messages);
    match low.logging {
