...
use super::{SearchManager, SearchSessionOptions, SearchType, CaseMode, SearchOutputMode};
...
pub struct StartSearchArgs {
    ...
    /// For content search: uses word boundary anchors (\b)
    /// For file search: requires exact filename match (not substring)
    #[serde(default)]
...
impl Tool for StartSearchTool {
    ...
    fn description() -> &'static str {
        "Start a streaming search that can return results progressively.\n\n\
         ...
         - Word boundary (word_boundary=true): Match whole words only\n\
           * Content search: Uses \\b anchors (e.g., 'test' matches 'test()' but not 'testing')\n\
           * File search: Requires exact filename match (e.g., 'lib' matches 'lib.rs' but not 'libtest.rs')\n\
         ...
         - word_boundary: Match whole words only (default: false)\n\
         ...
         - Search whole words: search_type=\"content\", pattern=\"test\", word_boundary=true\n\
           (matches 'test()' and 'test ' but not 'testing' or 'attest')\n\
         - Find exact filename: search_type=\"files\", pattern=\"lib\", word_boundary=true\n\
    ...
    async fn execute(&self, args: Self::Args) -> Result<Value, McpError> {
        ...
        if args.only_matching && output_mode != SearchOutputMode::Full {
            log::warn!("only_matching with output_mode={:?}: non-Full modes don't have match text, ignoring only_matching", output_mode);
        }
