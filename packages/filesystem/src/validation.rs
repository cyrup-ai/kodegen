...
fn get_allowed_dirs(config: &kodegen_config::ServerConfig) -> &[String] {
    &config.allowed_directories
}

/// Get the list of denied directories from config
fn get_denied_dirs(config: &kodegen_config::ServerConfig) -> &[String] {
...
fn is_path_allowed(
    path_to_check: &str, 
    config: &kodegen_config::ServerConfig
) -> (bool, Option<String>) {
