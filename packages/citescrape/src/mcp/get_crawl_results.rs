...
impl GetCrawlResultsTool {
        ...
        for t in types {
            ...
            if !VALID_TYPES.contains(&lower.as_str()) {
                return Err(McpError::InvalidArguments(
                    format!("Invalid file_type '{}'. Valid: md, html, json, png", t)
                ));
            }
    ...
    async fn list_crawled_files(
        ...
        while let Some(entry) = entries.next_entry().await? {
            ...
            if path.is_dir() {
                ...
                if let Some(filename) = path.file_name().and_then(|n| n.to_str()) && filename == "manifest.json" {
                    continue;
                }
                ...
                let should_include = match file_types {
                    ...
                    _ => {
                        ...
                        ext_str == "md" || ext_str == "html" || ext_str == "json" || ext_str == "png"
                    }
                };
    ...
    async fn format_active_response(
        ...
        // Add message
        if let Some(obj) = response.as_object_mut() {
            obj.insert("message".to_string(), json!(format!(
    ...
    async fn format_manifest_response_with_types(
        ...
        if let CrawlStatus::Completed = manifest.status {
            ...
            if let Some(obj) = response.as_object_mut() {
                obj.insert("search_index_dir".to_string(), 
                ...
                if args.list_files {
                    let files = Self::list_crawled_files(&manifest.output_dir, file_types.as_deref()).await?;
        ...
        if let CrawlStatus::Failed { ref error } = manifest.status
            && let Some(obj) = response.as_object_mut() {
                ...
                obj.insert("message".to_string(), json!(format!("Crawl failed: {}", error)));
            }
    ...
    async fn execute(&self, args: Self::Args) -> Result<Value, McpError> {
        ...
        if let Some(ref output_dir_str) = args.output_dir {
            ...
            if ManifestManager::exists(&output_dir).await {
                ...
                return Self::format_manifest_response_with_types(&manifest, &args, file_types).await;
            }
        }
