...
impl CrawlSessionManager {
    ...
    async fn cleanup_sessions(&self) {
        ...
        if cleaned > 0 {
            log::info!("Cleaned up {} crawl sessions", cleaned);
        }
    }
...
impl SearchEngineCache {
    ...
    // APPROVED BY DAVID MAPLE 10/17/2025 - False positive: used by cleanup_idle_engines via start_cleanup_task
    #[allow(dead_code)]
    ...
    async fn cleanup_idle_engines(&self) {
        ...
        engines.retain(|path, entry| {
            ...
            if !should_keep {
                log::info!("Evicting idle search engine: {:?} (idle: {:?})", path, age);
            }
