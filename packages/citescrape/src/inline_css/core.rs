...
type ResourceFuture = std::pin::Pin<Box<dyn std::future::Future<Output = Result<(String, String, ResourceType), InliningError>> + Send>>;
...
#[inline]
pub fn inline_all_resources(
    html_content: String,
    base_url: String,
    config: &InlineConfig,
    max_inline_image_size_bytes: Option<usize>,
    rate_rps: Option<f64>,
    on_result: impl FnOnce(Result<InliningResult>) + Send + 'static,
) -> AsyncTask<()> {
    ...
    spawn_async(async move {
        let result = async {
            ...
            let (css_downloads, img_downloads, svg_downloads) = join!(
                ...
                download_all_images(images, client.clone(), &config, max_inline_image_size_bytes, rate_rps),
...
async fn download_all_images(
    ...
    let futures = images.into_iter().map(|(image_url, src)| {
        ...
        async move {
            ...
            match super::downloaders::download_and_encode_image_async(image_url, client, &config, max_inline_size_bytes).await {
...
/// Returns InliningResult containing the processed HTML along with success/failure metrics.
#[inline]
pub fn inline_resources_from_info(
    html_content: String,
    base_url: String,
    config: &InlineConfig,
    ...
    spawn_async(async move {
        let result = async {
            ...
            log::debug!("Found {} stylesheets to process", resources.stylesheets.len());
                ...
                if !stylesheet.inline
                    && let Some(ref href) = stylesheet.url {
                        ...
                        let future = Box::pin(async move {
                            ...
                            match super::downloaders::download_css_async(css_url.clone(), client_clone, &config_clone).await {
                                Ok(content) => {
            ...
            for image in &resources.images {
                ...
                if is_svg {
                    ...
                    let future = Box::pin(async move {
                        ...
                        match super::downloaders::download_svg_async(svg_url.clone(), client_clone, &config_clone).await {
                            Ok(svg_content) => {
            ...
            for result in results {
                match result {
                    Ok((original_url, content, resource_type)) => {
                        match resource_type {
                            ResourceType::Css => {
                    ...
                    Err(inlining_error) => {
                        log::warn!("Failed to download {} from {}: {}", 
