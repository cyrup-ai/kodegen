...
pub fn apply_all_replacements(
    ...
    if !css_map.is_empty() {
        ...
        let matches: Vec<_> = document
            .select(css_selector)
            .map_err(|_| anyhow::anyhow!("Invalid CSS selector"))?
            .collect();
    ...
    document.serialize(&mut html_output)
        .context("Failed to serialize HTML after applying all replacements")?;
    
    String::from_utf8(html_output)
        .context("Failed to convert HTML bytes to UTF-8 string")
}
