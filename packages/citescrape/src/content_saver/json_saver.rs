...
pub async fn save_json_data(
    ...
    let _metadata = save_compressed_file(json_str.into_bytes(), &path, "application/json").await?;
...
pub async fn save_page_data(
    ...
    // save_compressed_file is now async
    let _metadata =
        save_compressed_file(json_content.into_bytes(), &path, "application/json").await?;
