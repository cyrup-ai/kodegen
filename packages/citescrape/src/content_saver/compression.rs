...
use crate::runtime::{spawn_async, TaskGuard};
...
/// 
/// Returns a TaskGuard that keeps the compression task alive. The guard must
/// be kept in scope until the operation completes (when on_result callback fires).
...
<body>
            ...
            match compress_with_level(&content, level) {
                Ok((duration, size)) => {
                    ...
                    println!(
                        ...
                        level, duration, size, ratio
                    );
                }
                Err(e) => println!("Level {}: Error - {}", level, e),
            }
        }
    }
    ...
    fn benchmark_compression_speedup() {
        ...
        if let (Some((time_3, size_3)), Some((time_9, size_9))) = (level_3, level_9) {
            ...
            println!("Size difference: Level 3 is {:.1}% larger", size_diff);
        }
    }
