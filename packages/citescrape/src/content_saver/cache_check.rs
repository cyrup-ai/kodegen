...
pub async fn read_cached_etag(url: &str, output_dir: &Path) -> Result<Option<String>> {
    ...
    tokio::task::spawn_blocking(move || -> Result<Option<String>> {
        ...
        let file = File::open(&gz_path)
            .context("Failed to open cached gzip file")?;
        ...
        let metadata: CacheMetadata = serde_json::from_str(comment)
            .context("Failed to parse cache metadata JSON")?;
