...
pub async fn crawl_pages<P: ProgressReporter>(
    ...
    // CRITICAL: Spawn handler task to drive the Chrome DevTools Protocol connection
    //
    // The handler returned by Browser::launch() MUST be continuously polled to:
    // - Process incoming CDP events from the browser
    // - Maintain WebSocket connection health
    // - Prevent protocol deadlocks and connection timeouts
    // - Handle error events from the browser process
    //
    // chromiumoxide documentation and API contract requires this pattern.
    // Dropping the handler without spawning causes:
    // - Lost protocol events
    // - Connection becoming unresponsive
    // - Potential resource leaks
    // - Intermittent browser failures under load
    //
    // Reference: chromiumoxide::Handler implements StreamExt and must be driven
    // See also: web_search/browser.rs for similar pattern
