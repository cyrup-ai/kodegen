...
#[inline(always)]
pub fn retry_task<F, T>(config: RetryConfig, mut operation: F) -> AsyncTask<SearchResult<T>>
where
...
#[inline(always)]
pub fn fallback_task<F, G, T>(primary: F, fallback: G) -> AsyncTask<SearchResult<T>>
where
...
impl<T> CancellableTask<T> {
        ...
        T: Send + 'static,
    {
        ...
        let task = spawn_async(async move { f(flag).await });

        Self { task, cancel_flag }
    }
