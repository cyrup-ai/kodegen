...
use anyhow::Result;
use tantivy::{
    query::{Query, TermQuery, BooleanQuery, PhraseQuery, FuzzyTermQuery, Occur},
    Term,
};
...
pub(crate) fn build_text_query_sync(
    engine: &SearchEngine,
    text: &str,
...
pub(crate) fn build_phrase_query_sync(
    ...
    if terms.len() == 1 {
        ...
        let title_terms: Vec<Term> = terms.iter()
    ...
    if terms.len() == 1 {
        ...
        let content_terms: Vec<Term> = terms.iter()
...
pub(crate) fn build_field_query_sync(
    ...
    if query.contains(' ') {
        ...
        let terms: Vec<Term> = query.split_whitespace()
            .map(|term| Term::from_field_text(field, term))
            .collect();
        
        if terms.len() == 1 {
            let term_query = TermQuery::new(terms[0].clone(), tantivy::schema::IndexRecordOption::WithFreqsAndPositions);
