...
use super::query_builders::{
    build_text_query_sync, build_phrase_query_sync, build_boolean_query_sync,
    build_field_query_sync, build_fuzzy_query_sync,
};
...
impl SearchQueryType {
    ...
    pub fn parse(query: &str) -> Self {
        ...
        if let Some(colon_pos) = query.find(':') {
            ...
            return SearchQueryType::Field { field, query: field_query };
        }

        // Check for fuzzy query (ends with ~ or ~N)
        if query.contains('~') && query.len() > 1
...
pub(crate) fn parse_query_sync(
    ...
    match query_type {
        SearchQueryType::Text(text) => 
            build_text_query_sync(engine, &text),
        SearchQueryType::Phrase(phrase) => 
            build_phrase_query_sync(engine, &phrase),
        SearchQueryType::Boolean(boolean_str) => 
            build_boolean_query_sync(engine, &boolean_str),
        SearchQueryType::Field { field, query } => 
            build_field_query_sync(engine, &field, &query),
