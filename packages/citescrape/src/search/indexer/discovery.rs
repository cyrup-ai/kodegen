    ...
    Box::new(WalkDir::new(root_dir)
        ...
        .process_read_dir(move |depth, path, _state, entries| {
            ...
            if let Some(dir_name) = path.file_name().and_then(|n| n.to_str())
                && (dir_name.starts_with('.') || 
                   matches!(dir_name, "node_modules" | "target" | "dist" | "build" | ".git" | "__pycache__")) {
                    entries.clear();
                    return;
                }
        ...
        .filter_map(move |entry| {
            match entry {
                ...
                Ok(_) => None, // Directory
                Err(e) if e.io_error().is_some_and(|io| io.kind() == std::io::ErrorKind::PermissionDenied) => {
...
fn extract_url_from_path_core(
    path: &str,
    root_len: usize,
) -> Result<Option<ImString>> {
    // Bounds check
...
fn is_valid_domain(domain: &str) -> bool {
    ...
    !domain.is_empty() && 
    domain.len() <= 253 && 
    domain.chars().all(|c| c.is_ascii() && (c.is_alphanumeric() || ".-".contains(c))) &&
    !domain.starts_with('.') && 
    !domain.ends_with('.') &&
    !domain.starts_with('-') && 
    !domain.ends_with('-') &&
