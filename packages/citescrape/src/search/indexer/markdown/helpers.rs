...
#[inline]
pub(crate) fn remove_list_marker(line: &str, indent_level: usize, list_depth: &mut usize) -> String {
    ...
    if let Some(dot_pos) = trimmed.find(". ") {
        ...
        if prefix.chars().all(|c| c.is_numeric()) && !prefix.is_empty() && prefix.len() <= 9 {
            *list_depth = (indent_level / 2) + 1;
    ...
    if let Some(paren_pos) = trimmed.find(") ") {
        ...
        if (prefix.chars().all(|c| c.is_numeric()) || 
            (prefix.len() == 1 && prefix.chars().next().is_some_and(|c| c.is_alphabetic()))) 
            && !prefix.is_empty() {
            *list_depth = (indent_level / 2) + 1;
...
#[inline]
pub(crate) fn remove_list_marker_inplace(buffer: &mut String, indent_level: usize, list_depth: &mut usize) {
