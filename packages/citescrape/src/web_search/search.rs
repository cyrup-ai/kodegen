...
///
/// Applies kromekover stealth features to the page before navigating to Google,
/// then performs the search by typing the query and clicking the search button.
///
/// # Arguments
...
/// - packages/citescrape/src/google_search.rs:136-177 (search flow)
pub async fn perform_search(page: &Page, query: &str) -> Result<()> {
    ...
    // NOW navigate to Google with stealth features active
    info!("Navigating to search homepage");
    page.goto(SEARCH_URL).await?;

    // Wait for page to be fully ready
    page.wait_for_navigation()
        .await
        .context("Failed to wait for page to load")?;

    info!("Page ready, finding search box");
    let search_box = page
    ...
    page.wait_for_navigation()
        .await
        .context("Failed to wait for search results")?;
