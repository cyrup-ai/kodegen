use kodegen_citescrape::crawl_events::*;
...
async fn test_publish_with_no_subscribers() {
    ...
    assert!(result.is_err(), "Publishing to empty bus should return error");
    match result {
        Err(kodegen_citescrape::crawl_events::EventBusError::NoSubscribers) => {
            // Expected behavior
        }
...
async fn test_subscribe_and_publish() {
    ...
    match (&event, &received) {
        (CrawlEvent::CrawlStarted { start_url: url1, .. }, CrawlEvent::CrawlStarted { start_url: url2, .. }) => {
            assert_eq!(url1, url2);
        }
...
fn test_event_creation_helpers() {
    let start_event = CrawlEvent::crawl_started(
    ...
    match start_event {
        CrawlEvent::CrawlStarted { start_url, max_depth, .. } => {
    ...
    match page_event {
        CrawlEvent::PageCrawled { url, depth, metadata, .. } => {
...
async fn test_filtered_receiver() {
    ...
    let mut filtered_receiver = bus.subscribe_filtered(|event| {
        matches!(event, CrawlEvent::PageCrawled { .. })
    });
...
async fn test_filtered_receiver_would_receive() {
    ...
    let filtered_receiver = bus.subscribe_filtered(|event| {
        matches!(event, CrawlEvent::LinkRewriteCompleted { .. })
    });
    ...
    let link_event = CrawlEvent::link_rewrite_completed(
        "https://test.com/target".to_string(),
        5,
        20,
    );
...
fn test_event_bus_config() {
    let config = EventBusConfig {
        capacity: 500,
        backpressure_mode: kodegen_citescrape::crawl_events::config::BackpressureMode::default(),
...
async fn test_block_backpressure_no_race_condition() {
    ...
    use kodegen_citescrape::crawl_events::config::BackpressureMode;
    ...
    for publisher_id in 0..num_publishers {
        ...
        let handle = tokio::spawn(async move {
            for event_id in 0..events_per_publisher {
                let event = CrawlEvent::page_crawled(
                    format!("https://example.com/publisher{}/page{}", publisher_id, event_id),
                    PathBuf::from(format!("/output/p{}_e{}.html", publisher_id, event_id)),
                    1,
                    PageCrawlMetadata {
                        html_size: 1024,
                        compressed_size: 512,
                ...
                match bus_clone.publish_with_backpressure(event).await {
                    Ok(_) => {},
                    Err(e) => panic!("Publisher {} event {} failed: {:?}", publisher_id, event_id, e),
                }
            }
        });
    ...
    let receiver_handle = tokio::spawn(async move {
        ...
        for i in 0..total_events {
            match timeout(Duration::from_secs(10), receiver.recv()).await {
                ...
                Err(_) => {
                    panic!("Timeout receiving event {} (received {}/{})", i, received_events.len(), total_events);
                }
            }
        }
    ...
    for (idx, handle) in publisher_handles.into_iter().enumerate() {
        match timeout(Duration::from_secs(10), handle).await {
            Ok(Ok(())) => {},
            Ok(Err(e)) => panic!("Publisher {} panicked: {:?}", idx, e),
            Err(_) => panic!("Publisher {} timed out", idx),
        }
    }
    ...
    println!("✅ Race condition test passed: {} events published and received with no drops", total_events);
}
