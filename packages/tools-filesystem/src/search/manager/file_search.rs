...
use ignore::{ParallelVisitor, ParallelVisitorBuilder, DirEntry};
use std::sync::Arc;
    ...
    fn visit(&mut self, entry: Result<DirEntry, ignore::Error>) -> ignore::WalkState {
        ...
        let matches = if let Some(ref glob) = self.glob_pattern {
            ...
            if self.word_boundary {
                ...
                Self::matches_with_word_boundary(file_name, &self.pattern, &self.pattern_lower, self.case_mode, self.is_pattern_lowercase)
            } else {
...
pub(super) fn execute(
    ...
    use super::utils::{build_type_changes, convert_case_mode, configure_walker};
    use std::sync::Arc;
    ...
    let glob_pattern = if options.literal_search {
        ...
        globset::Glob::new(&options.pattern).ok().map(|g| g.compile_matcher())
    };
    ...
    let mut builder = FileSearchBuilder {
        ...
        word_boundary: matches!(options.boundary_mode, Some(super::super::types::BoundaryMode::Word)),
        max_results,
