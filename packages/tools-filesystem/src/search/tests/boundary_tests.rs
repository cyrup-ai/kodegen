...
fn test_word_boundary_literal_search() {
    ...
    assert!(
        matcher.is_match(b"test ").expect("Match check failed"),
        "Should match 'test '"
    );
    assert!(
        matcher.is_match(b"test.").expect("Match check failed"),
    ...
    assert!(
        !matcher.is_match(b"testing").expect("Match check failed"),
        "Should not match 'testing'"
    );
    assert!(
        !matcher.is_match(b"attest").expect("Match check failed"),
        "Should not match 'attest'"
    );
    assert!(
        !matcher.is_match(b"fastest").expect("Match check failed"),
        "Should not match 'fastest'"
    );
    assert!(
        !matcher.is_match(b"libtest").expect("Match check failed"),
...
fn test_word_boundary_regex_search() {
    ...
    assert!(
        matcher.is_match(b"tester ").expect("Match check failed"),
        "Should match 'tester'"
    );
...
fn test_word_boundary_with_special_chars() {
    ...
    assert!(
        matcher.is_match(b"test.log").expect("Match check failed"),
    ...
    assert!(
        !matcher.is_match(b"testXlog").expect("Match check failed"),
        "Should not match 'testXlog'"
    );

    // Should NOT match when pattern is part of longer word
    assert!(
        !matcher.is_match(b"mytest.log").expect("Match check failed"),
...
fn test_word_boundary_with_numbers() {
    ...
    assert!(
        !matcher.is_match(b"test123").expect("Match check failed"),
        "Should NOT match 'test123' (no boundary)"
    );
    assert!(
        !matcher.is_match(b"123test").expect("Match check failed"),
        "Should NOT match '123test' (no boundary)"
    );

    // But boundaries exist with non-word characters
    assert!(
        matcher.is_match(b"test-123").expect("Match check failed"),
