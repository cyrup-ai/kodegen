// Category SSE Server: Filesystem Tools
//
// This binary serves only filesystem-related tools over SSE/HTTPS transport.
...
#[derive(Parser, Debug)]
#[command(name = "kodegen-filesystem")]
#[command(about = "Filesystem tools SSE server")]
struct Args {
    /// SSE server bind address (e.g., "127.0.0.1:30440")
    #[arg(long)]
    sse: SocketAddr,
...
impl ServerHandler for FilesystemServer {
    fn get_info(&self) -> ServerInfo {
        ServerInfo {
            ...
            instructions: Some("KODEGEN Filesystem Category Server".to_string()),
        }
    }
...
async fn main() -> Result<()> {
    ...
    let instance_id = chrono::Utc::now().format("%Y%m%d-%H%M%S-filesystem").to_string();
    ...
    // Build routers with only filesystem tools
    let tool_router = ToolRouter::new();
    let prompt_router = PromptRouter::new();

    // Register all filesystem tools
    let (tool_router, prompt_router) = register_filesystem_tools(
        tool_router,
        prompt_router,
        &config_manager,
        &usage_tracker,
    ...
    log::info!("Starting filesystem category SSE server on {protocol}://{}", args.sse);
...
/// Register all filesystem tools into routers
fn register_filesystem_tools<S>(
    ...
    S: Send + Sync + 'static,
{
    ...
    let file_read_line_limit = config_manager.get_file_read_line_limit();
    let _file_write_line_limit = config_manager.get_file_write_line_limit();
    ...
    // Register all filesystem tools
    let (tool_router, prompt_router) = register(
        tool_router,
        prompt_router,
        ReadFileTool::new(file_read_line_limit, config_manager.clone()),
    );

    let (tool_router, prompt_router) = register(
        tool_router,
        prompt_router,
        WriteFileTool::new(config_manager.clone()),
    );

    let (tool_router, prompt_router) = register(
        tool_router,
        prompt_router,
        EditBlockTool::new(config_manager.clone()),
    );

    let (tool_router, prompt_router) = register(
        tool_router,
        prompt_router,
        ListDirectoryTool::new(config_manager.clone()),
    );

    let (tool_router, prompt_router) = register(
        tool_router,
        prompt_router,
        CreateDirectoryTool::new(config_manager.clone()),
    );

    let (tool_router, prompt_router) = register(
        tool_router,
        prompt_router,
        MoveFileTool::new(config_manager.clone()),
    );

    let (tool_router, prompt_router) = register(
        tool_router,
        prompt_router,
        GetFileInfoTool::new(config_manager.clone()),
    );

    // Search tools use SearchManager
    let (tool_router, prompt_router) = register(
        tool_router,
        prompt_router,
    ...
    let (tool_router, prompt_router) = register(
        tool_router,
        prompt_router,
        ReadMultipleFilesTool::new(file_read_line_limit, config_manager.clone()),
    );

    let (tool_router, prompt_router) = register(
        tool_router,
        prompt_router,
        DeleteFileTool::new(config_manager.clone()),
    );

    let (tool_router, prompt_router) = register(
        tool_router,
        prompt_router,
        DeleteDirectoryTool::new(config_manager.clone()),
    );
