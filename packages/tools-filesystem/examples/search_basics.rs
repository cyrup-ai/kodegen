...
use kodegen_tools_filesystem::search::types::{SearchSessionOptions, SearchType, CaseMode, BinaryMode};
use std::fs;
...
async fn test_content_search(manager: &SearchManager, test_dir: &TempDir) -> Result<()> {
    ...
    let results = manager.get_more_results(&response.session_id, 0, 1000).await
    ...
    println!("✓ Found {} results in {} files", results.total_results, results.results.len());
    println!();
    Ok(())
}
...
async fn test_file_search(manager: &SearchManager, test_dir: &TempDir) -> Result<()> {
    ...
    if results.total_results < 3 {
        anyhow::bail!("Expected at least 3 .txt files, got {}", results.total_results);
    }
...
async fn test_nested_files(manager: &SearchManager, test_dir: &TempDir) -> Result<()> {
    ...
    let has_nested = results.results.iter().any(|r| r.file.contains("subdir") && r.file.contains("nested.txt"));
    if !has_nested {
