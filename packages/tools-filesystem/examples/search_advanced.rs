...
use kodegen_tools_filesystem::search::types::{SearchSessionOptions, SearchType, CaseMode, SearchOutputMode, EngineChoice, BinaryMode};
use std::fs;
...
fn create_test_environment() -> Result<TempDir> {
    ...
    fs::write(base.join("pcre_pattern.txt"), "before test123 after\ntest456\n")?;
    ...
    let binary_content = vec![b'h', b'e', b'l', b'l', b'o', 0x00, b'w', b'o', b'r', b'l', b'd'];
...
async fn test_engine_selection(manager: &SearchManager, test_dir: &TempDir) -> Result<()> {
    ...
    let results_auto = manager.get_more_results(&response_auto.session_id, 0, 1000).await?;
    ...
    let results_rust = manager.get_more_results(&response_rust.session_id, 0, 1000).await?;
    ...
    let results_pcre = manager.get_more_results(&response_pcre.session_id, 0, 1000).await?;
...
async fn test_pagination(manager: &SearchManager, test_dir: &TempDir) -> Result<()> {
    ...
    let response = manager.start_search(default_options(test_dir, "match")).await?;
    
    // Get first 10 results
    let page1 = manager.get_more_results(&response.session_id, 0, 10).await?;
    
    // Get next 10 results
    let page2 = manager.get_more_results(&response.session_id, 10, 10).await?;
    
    // Get last 10 results (using negative offset)
    let last_page = manager.get_more_results(&response.session_id, -10, 10).await?;
    
    println!("  Page 1 (0-10): {} results", page1.returned_count);
    println!("  Page 2 (10-20): {} results", page2.returned_count);
    println!("  Last page (last 10): {} results", last_page.returned_count);
...
async fn test_timeouts(manager: &SearchManager, test_dir: &TempDir) -> Result<()> {
    ...
    let results_timeout = manager.get_more_results(&response_timeout.session_id, 0, 1000).await?;
    ...
    let results_early = manager.get_more_results(&response_early.session_id, 0, 1000).await?;
    
    println!("  With timeout (5s): {} results", results_timeout.total_results);
    println!("  With early termination: {} results", results_early.total_results);
...
async fn test_binary_mode(manager: &SearchManager, test_dir: &TempDir) -> Result<()> {
    ...
    let results_binary = manager.get_more_results(&response_binary.session_id, 0, 1000).await?;
    ...
    let results_text = manager.get_more_results(&response_text.session_id, 0, 1000).await?;
    
    println!("  Auto mode: {} results (skips binary)", results_auto.total_results);
    println!("  Binary mode: {} results (searches binary, suppresses content)", results_binary.total_results);
    println!("  Text mode: {} results (treats all as text)", results_text.total_results);
...
async fn test_max_results(manager: &SearchManager, test_dir: &TempDir) -> Result<()> {
    ...
    // Unlimited results
    let response_unlimited = manager.start_search(default_options(test_dir, "match")).await?;
    let results_unlimited = manager.get_more_results(&response_unlimited.session_id, 0, 10000).await?;
    ...
    let results_limited = manager.get_more_results(&response_limited.session_id, 0, 10000).await?;
    ...
    if results_limited.total_results > 10 {
        anyhow::bail!("Expected max_results to limit to 10, got {}", results_limited.total_results);
    }
