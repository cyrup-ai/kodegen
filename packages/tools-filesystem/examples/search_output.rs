...
use kodegen_tools_filesystem::search::types::{SearchSessionOptions, SearchType, CaseMode, SearchOutputMode, EngineChoice, BinaryMode, SortBy, SortDirection};
use std::fs;
...
fn create_test_environment() -> Result<TempDir> {
    ...
    fs::write(base.join("context.txt"),
        "line1\nline2 match here\nline3\nline4 match here\nline5\n")?;
    
    // File with multiple matches
    fs::write(base.join("multi_match.txt"),
        "first match\nsome text\nsecond match\nmore text\nthird match\n")?;
    ...
    fs::write(base.join("partial.txt"),
        "prefix MATCH suffix\nanother MATCH line\n")?;
