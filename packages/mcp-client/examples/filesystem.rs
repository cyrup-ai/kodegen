...
async fn main() -> anyhow::Result<()> {
    ...
    let log_path = std::path::PathBuf::from("/Volumes/samsung_t9/kodegen/tmp/mcp-client/filesystem.log");
...
async fn run_filesystem_example(client: &common::LoggingClient) -> anyhow::Result<()> {
    ...
    let response: StartSearchResponse = client.call_tool_typed(
        tools::START_SEARCH,
        json!({
    ...
    // 10. GET_MORE_SEARCH_RESULTS - Get search results
    {
        ...
        client.call_tool(tools::LIST_SEARCHES, json!({}))
            .await
...
async fn cleanup_filesystem_resources(client: &common::LoggingClient, test_dir: &std::path::Path) {
    ...
    if !test_dir.exists() {
        info!("✅ Test directory already cleaned up: {}", test_dir.display());
        return;
    }
