...
async fn wait_for_crawl_completion(
    ...
    loop {
        let result = client.call_tool(tools::GET_CRAWL_RESULTS, json!({
            "crawl_id": crawl_id,
...
async fn main() -> Result<()> {
    ...
    let log_path = std::path::PathBuf::from("/Volumes/samsung_t9/kodegen/tmp/mcp-client/citescrape.log");
    if log_path.exists() {
...
async fn run_citescrape_example(client: &common::LoggingClient) -> Result<()> {
    ...
    let test_result = async {
        ...
        let response: StartCrawlResponse = client.call_tool_typed(tools::START_CRAWL, json!({
        ...
        tracing::info!("Starting crawl with invalid domain (expecting failure due to zero pages)...");
        
        // start_crawl succeeds (returns crawl_id) because validation only checks URL syntax
        let response_invalid: StartCrawlResponse = client.call_tool_typed(tools::START_CRAWL, json!({
        ...
        let failure_result = wait_for_crawl_completion(
            client, 
            &crawl_id_invalid, 
        ...
        match failure_result {
            Ok(()) => anyhow::bail!("Expected crawl to fail for invalid domain, but it reported success"),
            Err(e) => {
                tracing::info!("✅ Correctly handled invalid domain with zero-page detection: {}", e);
        ...
        let result = client.call_tool(tools::SEARCH_CRAWL_RESULTS, json!({
            "crawl_id": crawl_id,
            "query": "ratatui",
            "limit": 5
        })).await?;
        ...
        match client.call_tool(tools::WEB_SEARCH, json!({
        ...
        let response_2: StartCrawlResponse = client.call_tool_typed(tools::START_CRAWL, json!({
            "url": "https://tokio.rs/",
            "limit": 3,
            "max_depth": 1,
            "allow_subdomains": false
        ...
        let _result = client.call_tool(tools::GET_CRAWL_RESULTS, json!({
            "crawl_id": crawl_id_2,
            "include_progress": false,
            "list_files": false
        })).await?;
    ...
    if !crawl_ids.is_empty() {
        tracing::info!("\nCrawl sessions will auto-cleanup after 5 minutes: {:?}", crawl_ids);
    }
