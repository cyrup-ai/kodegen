...
static ANALYSIS_CACHE: Lazy<Mutex<LruCache<String, CharCodeData>>> = Lazy::new(|| {
...
impl CharCodeData {
    ...
    fn analyze_uncached(expected: &str, actual: &str) -> Self {
        ...
        let has_zero_width = ZERO_WIDTH_CHARS.iter().any(|&code| codes.contains_key(&code));
...
fn find_common_boundaries(a: &str, b: &str) -> (usize, usize) {
    ...
    let suffix_len = a_chars.iter().rev()
...
fn format_char_display(code: u32) -> String {
    match code {
        32..=126 => {
            char::from_u32(code)
                .map(|c| c.to_string())
                .unwrap_or_else(|| format!("\\x{:02x}", code))
        }
        _ => format!("\\x{:02x}", code),
    }
}
...
fn detect_whitespace_issues(
    ...
    if expected_diff.lines().any(|line| line.ends_with(' ') || line.ends_with('\t'))
        || actual_diff.lines().any(|line| line.ends_with(' ') || line.ends_with('\t'))
    {
    ...
    if let Some(issue) = whitespace_issues.first() {
        match issue {
            WhitespaceIssue::TabsVsSpaces => {
                return Some("Replace tabs with spaces (or vice versa) in your search string".to_string());
            }
            ...
            WhitespaceIssue::TrailingWhitespace => {
                return Some("Remove trailing whitespace from lines in your search string".to_string());
            }
        }
    }
...
fn format_visual_diff_with_codes(
    ...
    let suffix: String = exp_chars.iter().skip(exp_chars.len().saturating_sub(suffix_len)).collect();
    ...
    output.push_str(&format!("\nExpected diff: {:?} [{}]", expected_diff, inline_codes(expected_diff)));
    output.push_str(&format!("\nActual diff:   {:?} [{}]", actual_diff, inline_codes(actual_diff)));
...
impl CharCodeData {
    ...
    pub fn format_detailed_report(&self) -> String {
        ...
        output.push_str(&format!("  Unique codes: {}, Diff length: {}\n", self.unique_count, self.diff_length));
        
        // Classification
        if !self.classification.whitespace.is_empty() || !self.classification.line_endings.is_empty() {
            output.push_str("\nCharacter Types:\n");
            
            if !self.classification.whitespace.is_empty() {
                let ws: Vec<String> = self.classification.whitespace.iter()
            ...
            if !self.classification.line_endings.is_empty() {
                let le: Vec<String> = self.classification.line_endings.iter()
                    .map(|(code, count)| format!("{}×{}", format_char_name(*code), count))
                    .collect();
            ...
            if !self.classification.control.is_empty() {
                output.push_str(&format!("  Control chars: {} types\n", self.classification.control.len()));
            }
            
            if !self.classification.unicode.is_empty() {
                output.push_str(&format!("  Unicode chars: {} types\n", self.classification.unicode.len()));
            }
        }
        
        // Issues detected
        if !self.whitespace_issues.is_empty() || !self.encoding_issues.is_empty() || self.has_zero_width {
            output.push_str("\nIssues Detected:\n");
        ...
        // Distribution
        if !self.distribution.only_in_expected.is_empty() || !self.distribution.only_in_actual.is_empty() {
            output.push_str("\nDistribution:\n");
            
            if !self.distribution.only_in_expected.is_empty() {
                let chars: Vec<String> = self.distribution.only_in_expected.iter()
                    .map(|(code, count)| format!("{}×{}", format_char_name(*code), count))
                    .collect();
            ...
            if !self.distribution.only_in_actual.is_empty() {
                let chars: Vec<String> = self.distribution.only_in_actual.iter()
                    .map(|(code, count)| format!("{}×{}", format_char_name(*code), count))
                    .collect();
