...
impl Suggestion {
    ...
    pub fn for_failure(reason: &EditFailureReason, context: &SuggestionContext) -> Self {
        match reason {
            EditFailureReason::FuzzyMatchAboveThreshold { similarity, is_whitespace_only } => {
                Self::fuzzy_match_above_threshold(*similarity, *is_whitespace_only, context)
            }
            
            EditFailureReason::FuzzyMatchBelowThreshold { similarity, threshold, found_text } => {
    ...
    fn fuzzy_match_above_threshold(
        ...
        if is_whitespace_only {
            actions.push("The difference is only whitespace - check for extra/missing spaces or tabs".to_string());
        }
        ...
        if let Some(time_ms) = context.execution_time_ms {
            message.push_str(&format!(" (found in {:.2}ms)", time_ms));
        }
    ...
    fn fuzzy_match_below_threshold(
        ...
        // Add execution time if available (EDIT_06 hook)
        if let Some(time_ms) = context.execution_time_ms {
            message.push_str(&format!(" (search completed in {:.2}ms)", time_ms));
        }
    ...
    fn unexpected_count(expected: usize, found: usize, _context: &SuggestionContext) -> Self {
        ...
        let actions = if found > expected {
            ...
            vec![
                ...
                format!("Update expected_replacements to {} to match actual count", found),
            ]
        };
    ...
    fn empty_search() -> Self {
        Self {
            ...
            actions: vec![
                "Provide a non-empty string to search for".to_string(),
            ],
        }
    }
    ...
    fn identical_strings() -> Self {
        Self {
            ...
            actions: vec![
                "No changes would be made - provide different strings".to_string(),
            ],
        }
    }
}
