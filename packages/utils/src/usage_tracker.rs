...
impl UsageTracker {
    /// Create new `UsageTracker` with stats file in ~/.kodegen/stats.json
    #[must_use]
    pub fn new() -> Self {
        let stats_file = Self::get_stats_file_path();
    ...
    fn get_stats_file_path() -> PathBuf {
        if let Some(home) = dirs::home_dir() {
            home.join(".kodegen").join("stats.json")
        } else {
            PathBuf::from("stats.json") // Fallback
        }
    }
    ...
    fn get_category(tool_name: &str) -> Option<&'static str> {
        match tool_name {
            ...
            | "list_directory" | "move_file" | "delete_file" | "delete_directory"
            | "get_file_info" | "search_files" => Some("filesystem"),
            ...
            "start_search" | "get_more_results" 
            | "stop_search" | "list_searches" => Some("search"),

            "get_usage_stats" | "get_config" | "set_config_value" 
            | "get_prompts" | "get_recent_tool_calls" => Some("config"),
    ...
    pub fn track_success(&self, tool_name: &str) {
        let _ = self.update_sender.send(StatsUpdate::Success(tool_name.to_string()));
    }

    /// Track a failed tool call (fire-and-forget, never blocks)
    pub fn track_failure(&self, tool_name: &str) {
        let _ = self.update_sender.send(StatsUpdate::Failure(tool_name.to_string()));
    }
    ...
    pub fn get_summary(&self) -> String {
        ...
        let success_rate = if stats.total_tool_calls > 0 {
            f64::from(u32::try_from(stats.successful_calls).unwrap_or(u32::MAX)) 
                / f64::from(u32::try_from(stats.total_tool_calls).unwrap_or(u32::MAX)) * 100.0
        } else {
            0.0
        };
