...
fn get_wix_toolset() -> Result<PathBuf> {
    ...
    // Download and verify
    let data = http::download_and_verify(WIX_URL, WIX_SHA256, http::HashAlgorithm::Sha256)?;

    log::info!("Extracting WiX toolset to {}", wix_path.display());

    // Create cache directory
    std::fs::create_dir_all(&cache_dir).fs_context("creating cache directory", &cache_dir)?;
...
fn build_msi(settings: &Settings, wix_path: &Path) -> Result<Vec<PathBuf>> {
    ...
    let arch = match settings.binary_arch() {
        ...
        _ => bail!(
            "Unsupported architecture for MSI: {:?}",
    ...
    if sign::should_sign(settings) {
        sign::sign_file(&msi_path, settings).context("signing MSI installer")?;
    }
...
fn generate_wxs(settings: &Settings, output_dir: &Path, arch: &str) -> Result<PathBuf> {
    ...
    let win64 = if arch == "x64" || arch == "arm64" {
        "yes"
    } else {
        "no"
    };
    ...
    let binaries: Vec<BTreeMap<String, String>> = settings
        .binaries()
        .iter()
        .map(|bin| {
            ...
            bin_data.insert("is_main".to_string(), bin.main().to_string());
            ...
            bin_data.insert("component_id".to_string(), format!("Binary_{}", bin.name()));
            bin_data.insert("file_id".to_string(), format!("File_{}", bin.name()));
    ...
    std::fs::write(&wxs_path, wxs_content).fs_context("writing WiX source file", &wxs_path)?;
...
fn run_light(
    ...
    let output = windows_command(&light)
        .args(&[
            ...
            wixobj_str,
            "-ext",
            "WixUIExtension", // UI components
            "-ext",
            "WixUtilExtension", // Utility components
