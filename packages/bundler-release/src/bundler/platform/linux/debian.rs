...
pub fn bundle_project(settings: &Settings) -> Result<Vec<PathBuf>> {
    ...
    let data_dir =
        generate_data(settings, &package_dir).context("failed to generate data directory")?;
    ...
    generate_scripts(settings, &control_dir).context("failed to generate control scripts")?;
    generate_md5sums(&control_dir, &data_dir).context("failed to generate md5sums file")?;
    ...
    let control_tar_gz =
        tar_and_gzip_dir(control_dir).context("failed to tar/gzip control directory")?;
    let data_tar_gz = tar_and_gzip_dir(data_dir).context("failed to tar/gzip data directory")?;
...
fn generate_desktop_file(settings: &Settings, data_dir: &Path) -> Result<()> {
    ...
    let mut file = fs::create_file(&desktop_path).context("failed to create desktop file")?;
...
fn generate_changelog(settings: &Settings, data_dir: &Path) -> Result<()> {
    if let Some(changelog_path) = &settings.bundle_settings().deb.changelog {
        ...
        let mut src =
            File::open(changelog_path).fs_context("opening changelog file", changelog_path)?;
        let dest_file = fs::create_file(&dest).context("failed to create changelog destination")?;
...
fn generate_control_file(
    ...
    let mut file = fs::create_file(&control_path).context("failed to create control file")?;
...
fn generate_md5sums(control_dir: &Path, data_dir: &Path) -> Result<()> {
    ...
    let mut file = fs::create_file(&md5sums_path).context("failed to create md5sums file")?;
    ...
    for entry in WalkDir::new(data_dir) {
        ...
        // Calculate MD5 hash
        let mut src = File::open(entry.path()).fs_context("opening file for MD5", entry.path())?;
...
fn generate_scripts(settings: &Settings, control_dir: &Path) -> Result<()> {
    ...
    for (script_opt, name) in scripts {
        if let Some(script_path) = script_opt {
            ...
            let mut src = File::open(script_path).fs_context("opening script file", script_path)?;
            ...
            {
                let mut dest_file =
                    File::create(&dest).fs_context("creating script file", &dest)?;
                io::copy(&mut src, &mut dest_file)?;
            }
        }
    }
...
fn tar_and_gzip_dir(src_dir: PathBuf) -> Result<PathBuf> {
    ...
    let tar_gz = File::create(&dest_path).fs_context("creating tar.gz file", &dest_path)?;
    ...
    for entry in WalkDir::new(&src_dir) {
        ...
        let metadata = std::fs::metadata(path).fs_context("reading metadata", path)?;
...
fn create_ar_archive(files: Vec<PathBuf>, dest: &Path) -> Result<()> {
    let dest_file = File::create(dest).fs_context("creating .deb archive", dest)?;
