        ...
        if *json {
            ...
            if *detailed {
                ...
                match crate::workspace::DependencyGraph::build(&workspace) {
                    Ok(dep_graph) => {
                        match dep_graph.publish_order() {
                            Ok(publish_order) => {
                                config.println(&format!("  Total packages: {} (in {} tiers)\n", 
                                    publish_order.total_packages,
                                    publish_order.tier_count()
                                ));
                                for tier in &publish_order.tiers {
                                    config.println(&format!("  Tier {} ({} package{}):",
                                        tier.tier_number,
                                        tier.packages.len(),
                                    ...
                                    for pkg in &tier.packages {
                                        ...
                                        config.println(&format!("    • {} ({} dependent{})",
                                            pkg,
                                            dependents.len(),
                            ...
                            Err(e) => {
                                config.error_println(&format!("Failed to compute publish order: {}", e));
                            }
                        }
                    }
                    Err(e) => {
