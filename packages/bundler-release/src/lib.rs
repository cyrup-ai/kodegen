...
pub use error::{Result, ReleaseError, CliError};
pub use workspace::{WorkspaceInfo, DependencyGraph, PublishOrder};
...
pub use bundler::{Bundler, BundleSettings, PackageType, BundledArtifact};
...
async fn get_origin_url(repo_path: &std::path::Path) -> Result<String> {
    let repo = kodegen_tools_git::open_repo(repo_path).await
        .map_err(|_| ReleaseError::Git(error::GitError::NotRepository))?
        .map_err(|_| ReleaseError::Git(error::GitError::NotRepository))?;
    
    let remotes = kodegen_tools_git::list_remotes(&repo).await
        .map_err(|e| ReleaseError::Git(error::GitError::RemoteOperationFailed {
            operation: "list_remotes".to_string(),
            reason: e.to_string(),
        }))?;
    let origin = remotes.iter()
        .find(|r| r.name == "origin")
        .ok_or_else(|| ReleaseError::Git(error::GitError::RemoteOperationFailed {
...
async fn clone_main_to_temp() -> Result<PathBuf> {
    ...
    let timestamp = std::time::SystemTime::now()
        ...
        .map_err(|e| ReleaseError::Cli(error::CliError::ExecutionFailed {
...
#[allow(dead_code)]
fn create_bundles(workspace: &workspace::WorkspaceInfo, version: &semver::Version) -> Result<Vec<crate::bundler::BundledArtifact>> {
    use crate::bundler::{Bundler, SettingsBuilder, PackageSettings, BundleBinary, BundleSettings, DebianSettings, RpmSettings};
    use crate::error::CliError;
    use std::path::PathBuf;
    ...
    let binaries = workspace
        ...
        .filter(|pkg| {
            ...
            manifest.map(|m| m.contains("[[bin]]") || 
                             (m.contains("name = ") && !m.contains("[lib]")))
                   .unwrap_or(false)
        })
        .map(|pkg| {
