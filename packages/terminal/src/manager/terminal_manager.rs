...
fn detect_repl_ready(output: &str) -> bool {
    ...
    if REPL_PROMPTS.iter().any(|prompt| last_line.ends_with(prompt)) {
        return true;
    }
...
impl TerminalManager {
    ...
    pub async fn spawn_command(&self, command: &str, shell_path: Option<&str>) -> Result<u32, anyhow::Error> {
    ...
    pub async fn get_output(
        ...
        let (lines, total_lines, start, end, is_complete) = {
            ...
            let lines: Vec<String> = screen.rows(0, cols)
                .skip(start)
                .take(end - start)
                .collect();
    ...
    pub async fn send_input(&self, pid: u32, input: &str, append_newline: bool) -> Result<bool, anyhow::Error> {
    ...
    pub async fn force_terminate(&self, pid: u32) -> Result<(), McpError> {
        ...
        let session = sessions.get(&pid)
            .ok_or_else(|| McpError::InvalidArguments(
                format!("No active session found for PID: {}", pid)
            ))?
            .clone();
        drop(sessions);
    ...
    pub async fn metrics(&self) -> TerminalMetrics {
        ...
        let average_duration = if count > 0 {
            total_duration_secs / count as f64
        } else {
            0.0
        };
        
        // Get total sessions created from atomic counter
        let total_created = self.next_pid.load(AtomicOrdering::SeqCst) as u64 - 1000;
    ...
    pub async fn cleanup_sessions(&self) {
        ...
        for (pid, session) in sessions.iter() {
            ...
            let is_complete = session.terminal
                .try_read()
                .map(|t| t.is_pty_closed())
                .unwrap_or(false);
            
            let last_read = session.last_read_time
                .try_read()
                .map(|t| *t)
                .unwrap_or(now);
        ...
        if !to_complete.is_empty() {
            ...
            log::info!("Moved {} sessions to completed_sessions", moved_count);
        }
    ...
    async fn cleanup_completed_sessions(&self) {
        ...
        completed.retain(|pid, session| {
            let age = now.duration_since(session.end_time).unwrap_or(Duration::ZERO);
            let should_keep = age < cutoff;
            
            if !should_keep {
                log::debug!("Removing old completed session PID {} (age: {:?})", pid, age);
            }
        ...
        if removed_count > 0 {
            log::info!("Cleaned up {} old completed sessions", removed_count);
        }
    }
