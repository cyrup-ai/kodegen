...
use kodegen_claude_agent::{HookEvent, PermissionRequest, PermissionResult, PermissionResultAllow, RequestId, ToolName, ToolPermissionContext};
use tokio::sync::mpsc;
...
async fn test_protocol_initialization() {
    ...
    assert!(init_req.capabilities.contains(kodegen_claude_agent::control::protocol::ClientCapabilities::BIDIRECTIONAL));
    assert!(init_req.capabilities.contains(kodegen_claude_agent::control::protocol::ClientCapabilities::HOOKS));
    assert!(init_req.capabilities.contains(kodegen_claude_agent::control::protocol::ClientCapabilities::PERMISSIONS));
    assert!(init_req.capabilities.contains(kodegen_claude_agent::control::protocol::ClientCapabilities::INTERRUPTS));

    // Simulate init response
    let init_response = InitResponse {
        protocol_version: "1.0".to_string(),
        cli_version: "1.0.0".to_string(),
        capabilities: kodegen_claude_agent::control::protocol::ServerCapabilities::all_features(),
...
async fn test_request_id_uniqueness() {
    ...
    let ControlRequest::SendMessage { id: id3, .. } = req3 else {
        panic!("Expected SendMessage request");
    };
...
async fn test_init_message_serialization() {
    ...
    match deserialized {
        ControlMessage::Init(init) => {
            ...
            assert!(init.capabilities.contains(kodegen_claude_agent::control::protocol::ClientCapabilities::BIDIRECTIONAL));
        }
...
async fn test_init_response_message_serialization() {
    ...
    match deserialized {
        ControlMessage::InitResponse(resp) => {
            ...
            assert_eq!(resp.session_id, "test_session");
        }
        _ => panic!("Wrong message type"),
    }
}
