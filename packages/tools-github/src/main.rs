// Category SSE Server: GitHub Tools
//
// This binary serves only GitHub API tools over SSE/HTTPS transport.
...
#[derive(Parser, Debug)]
#[command(name = "kodegen-github")]
#[command(about = "GitHub API tools SSE server")]
struct Args {
    /// SSE server bind address (e.g., "127.0.0.1:30451")
    #[arg(long)]
    sse: SocketAddr,
...
impl ServerHandler for GitHubServer {
    fn get_info(&self) -> ServerInfo {
        ServerInfo {
            ...
            instructions: Some("KODEGEN GitHub Category Server".to_string()),
        }
    }
...
async fn main() -> Result<()> {
    ...
    let instance_id = chrono::Utc::now().format("%Y%m%d-%H%M%S-github").to_string();
    ...
    // Build routers with only github tools
    let tool_router = ToolRouter::new();
    let prompt_router = PromptRouter::new();

    // Register all github tools (NO managers needed)
    let (tool_router, prompt_router) = register_github_tools(
        tool_router,
        prompt_router,
        &config_manager,
        &usage_tracker,
    ...
    log::info!("Starting github category SSE server on {protocol}://{}", args.sse);
    ...
    log::info!("GitHub server stopped");
    Ok(())
}

/// Register all GitHub tools into routers
/// 
/// CRITICAL: GitHub tools are ZERO-STATE structs - pass directly, NO ::new()
fn register_github_tools<S>(
    ...
    S: Send + Sync + 'static,
{
    ...
    // Issue tools (7)
    (tool_router, prompt_router) = register(tool_router, prompt_router, CreateIssueTool);
    (tool_router, prompt_router) = register(tool_router, prompt_router, GetIssueTool);
    (tool_router, prompt_router) = register(tool_router, prompt_router, ListIssuesTool);
    (tool_router, prompt_router) = register(tool_router, prompt_router, UpdateIssueTool);
    (tool_router, prompt_router) = register(tool_router, prompt_router, SearchIssuesTool);
    (tool_router, prompt_router) = register(tool_router, prompt_router, AddIssueCommentTool);
    (tool_router, prompt_router) = register(tool_router, prompt_router, GetIssueCommentsTool);

    // Pull Request tools (9)
    (tool_router, prompt_router) = register(tool_router, prompt_router, CreatePullRequestTool);
    (tool_router, prompt_router) = register(tool_router, prompt_router, UpdatePullRequestTool);
    (tool_router, prompt_router) = register(tool_router, prompt_router, MergePullRequestTool);
    (tool_router, prompt_router) = register(tool_router, prompt_router, GetPullRequestStatusTool);
    (tool_router, prompt_router) = register(tool_router, prompt_router, GetPullRequestFilesTool);
    (tool_router, prompt_router) = register(tool_router, prompt_router, GetPullRequestReviewsTool);
    (tool_router, prompt_router) = register(tool_router, prompt_router, CreatePullRequestReviewTool);
    (tool_router, prompt_router) = register(tool_router, prompt_router, AddPullRequestReviewCommentTool);
    (tool_router, prompt_router) = register(tool_router, prompt_router, RequestCopilotReviewTool);

    // Repository tools (2)
    (tool_router, prompt_router) = register(tool_router, prompt_router, CreateRepositoryTool);
    (tool_router, prompt_router) = register(tool_router, prompt_router, ForkRepositoryTool);

    // Branch/Commit tools (4)
    (tool_router, prompt_router) = register(tool_router, prompt_router, ListBranchesTool);
    (tool_router, prompt_router) = register(tool_router, prompt_router, CreateBranchTool);
    (tool_router, prompt_router) = register(tool_router, prompt_router, ListCommitsTool);
    (tool_router, prompt_router) = register(tool_router, prompt_router, GetCommitTool);

    // Search tools (3)
    (tool_router, prompt_router) = register(tool_router, prompt_router, SearchCodeTool);
    (tool_router, prompt_router) = register(tool_router, prompt_router, SearchRepositoriesTool);
    (tool_router, prompt_router) = register(tool_router, prompt_router, SearchUsersTool);
