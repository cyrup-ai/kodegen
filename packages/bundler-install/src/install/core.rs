...
impl InstallContext {
    ...
    pub fn validate_prerequisites(&self) -> Result<()> {
        // Check if running as appropriate user and escalate if needed
        #[cfg(unix)]
        {
            // System daemons always require root privileges
            let uid = unsafe { libc::getuid() };
            let already_elevated = std::env::var("SUDO_USER").is_ok();

            if uid != 0 && !already_elevated {
                // Attempt automatic privilege escalation
                eprintln!("🔐 System daemon installation requires elevated privileges");
                eprintln!("   Re-launching with sudo...");
                Self::escalate_with_sudo()?;
                // If we reach here, escalation failed
                return Err(anyhow::anyhow!(
                    "Privilege escalation failed. Please run with sudo manually."
                ));
            } else if uid != 0 && already_elevated {
                // We're in sudo context but still don't have root - this shouldn't happen
                return Err(anyhow::anyhow!(
                    "Running under sudo but UID is not 0. This is unexpected."
                ));
            }
        }
