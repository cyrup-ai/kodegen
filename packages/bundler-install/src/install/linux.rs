...
impl PlatformExecutor {
    ...
    pub fn install(b: InstallerBuilder) -> Result<(), InstallerError> {
        ...
        Self::create_systemd_unit_with_dir(&config, &unit_dir)?;
...
"#,
    ...
    fn enable_user_systemd_service(service_name: &str) -> Result<(), InstallerError> {
    ...
    fn start_user_systemd_service(service_name: &str) -> Result<(), InstallerError> {
        let output = Command::new("systemctl")
            .args(["--user", "start", &format!("{}.service", service_name)])
            .output()
            .map_err(|e| {
