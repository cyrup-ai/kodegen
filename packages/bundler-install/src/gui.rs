...
impl InstallWindow {
    ...
    fn show_progress_panel(&self, ui: &mut egui::Ui) {
        ...
        ui.label(
            egui::RichText::new(&self.current_step)
                .size(18.0)
                .strong()
                .color(egui::Color32::from_rgb(24, 202, 155)),
        ); // Cyan accent
        ...
        ui.label(
            egui::RichText::new(&self.current_message)
                .size(14.0)
...
pub async fn run_gui_installation(
    ...
    tokio::spawn(async move {
        ...
        let (_kodegen_path, kodegend_path) =
            match crate::get_bundled_binaries(cli_clone.from_platform.clone()).await {
                Ok(paths) => paths,
                Err(e) => {
