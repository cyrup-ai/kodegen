use super::metadata::{PromptMetadata, PromptTemplate, ParameterType};
...
pub fn parse_template(filename: &str, file_content: &str) -> Result<PromptTemplate> {
    ...
    let parsed: gray_matter::ParsedEntity<Pod> = matter.parse(file_content)
        .map_err(|e| anyhow::anyhow!("Failed to parse frontmatter: {e}"))?;
    
    // Extract and deserialize frontmatter
    let metadata: PromptMetadata = parsed.data
...
fn apply_defaults(
    ...
    for param_def in &template.metadata.parameters {
        if !params.contains_key(&param_def.name) && let Some(default) = param_def.default.as_ref() {
