...
pub(crate) async fn collect_dependency_metrics(
        ...
        .await
    {
        ...
        Err(e) => {
            warn!(
                "Failed to fetch Dependabot alerts for {}/{}: {} - defaulting to 0",
                owner, repo, e
            );
            0
        }
    ...
    let outdated_dependencies = if total_dependencies > 50 {
        ...
        match tokio::time::timeout(Duration::from_secs(10), check_future).await {
            Ok(count) => count,
            Err(_) => {
                warn!(
                    "Timeout checking outdated dependencies for {}/{}",
                    owner, repo
                );
                0
            }
