...
pub(crate) async fn analyze_repo(
    ...
    let url = repo.clone_url.as_ref().map(|u| u.as_str()).unwrap_or("");
    ...
    let owner_login = repo
        ...
        .ok_or_else(|| SearchError::InvalidQuery {
            details: format!(
                "Repository '{}' has no owner information - cannot fetch commits",
                repo_name
            ),
        })?
        .login
    ...
    // Compute API metrics
    let (api_score, _) = compute_api_metrics(&repo, &activity);
    ...
    let result = RepositoryResult {
        ...
        quality_metrics: QualityMetrics {
            ...
            security_score: local_scores
                .metrics
                .as_ref()
                .map(|m| calculate_security_score(&m.security_metrics))
                .unwrap_or(0.5),
        },
...
pub(crate) async fn compute_activity(
        ...
        .await
    {
        Ok(prs_page) => prs_page
            ...
            .filter(|pr| {
                pr.merged_at
                    .map(|merged_at| merged_at > thirty_days_ago)
                    .unwrap_or(false)
            })
            .count() as u32,
        Err(e) => {
        ...
        Ok(issues_page) => issues_page
            ...
            .filter(|issue| {
                issue.pull_request.is_none()
                    && issue
                        .closed_at
                        .map(|closed| closed > thirty_days_ago)
                        .unwrap_or(false)
            })
            .count() as u32,
        Err(e) => {
...
pub(crate) async fn local_analysis(
    ...
    let repo_size_kb = repo.size.unwrap_or(0) as u64;
    ...
    let overall_local = (readme_score + coverage_score) / 2.0;
...
fn calculate_signed_commits_ratio(repo: &gix::Repository) -> f32 {
    ...
    if let Ok(head) = repo.head()
        ...
        && let Ok(peeled) = head_ref.peel_to_id_in_place()
    {
        let walk = peeled
            .ancestors()
